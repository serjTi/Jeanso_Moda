...
public class DownloadABVXML extends AsyncTask<String, Void, String> {
    private GetProdactDatabase delegate;

    public DownloadABVXML(GetProdactDatabase listener) {
        delegate = listener;
    }
    ...
    protected void onPostExecute(String strXML) {
        ...
        if (delegate != null) {
            delegate.download_abv_xml_is_done(strXML);
        }
    }
}