...
public class DownloadTosXml extends AsyncTask<String, Void, String> {
    private GetProdactDatabase delegate;

    public DownloadTosXml(GetProdactDatabase listener) {
        delegate = listener;
    }
    ...
    protected void onPostExecute(String strXML) {
        ...
        if (delegate != null) {
            delegate.download_tos_xml_is_done(strXML);
        }
    }
}